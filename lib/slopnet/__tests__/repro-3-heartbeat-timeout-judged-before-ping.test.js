...
describe('REPRO: host heartbeat evaluates the timeout before sending its own ping', () => {
    ...
    it('does not silently drop the host\'s outbound game state to a live client', async () => {
        ...
        expect(
            ...
            `Host emitted: [${events.join(', ')}]`
            // CORRECTED EXPECTATION (was `aliceReceived: 1`): this test pushes the state
            // to Alice TWICE — once with the targeted send() and once with the
            // broadcast() on the next line — so a host that has stopped dropping her
            // traffic delivers two messages, not one. The bug being reproduced is that
            // she received ZERO; counting the deliveries correctly does not weaken that.
        ).toEqual({ sendResult: true, aliceReceived: 2 });
    ...
    it('does not lose the clientId of a live client once the reconnect window has closed', async () => {
        ...
        // Past the 120s reconnect window the buggy host deleted the record entirely
        // (:356-359) and emitted client-lost — an event no app subscribes to.
        //
        // CORRECTED PRECONDITION (was `expect(events).toContain('client-lost:alice')`):
        // that line asserted the BUG, and it contradicts the sibling test above, which
        // requires the single late wake to evict nobody. With the eviction gone there is
        // nothing for the reconnect window to expire, so the seat is still Alice's two
        // minutes later. The assertion this test exists for — that a live player's
        // message still resolves to her clientId — is untouched below.
        await vi.advanceTimersByTimeAsync(DEFAULT_CONFIG.reconnectWindowMs + 1000);
        expect(events).not.toContain('client-lost:alice');
        expect(host.getConnectedClientIds().sort()).toEqual(['alice', 'bob']);
        expect(alice.isConnected).toBe(true);
